    ...
    for i in range(5):
        ...
        while carry == None:
            ...
            if carry > len(old_house):
                ...
                truck_load.append(old_house[carry - 1])
                del old_house[carry - 1]
                currently_in_old_house()
...
def taking_out_of_Uhaul(truck_load):
    ...
    for i in range(len(truck_load)):
        item = truck_load.pop()
        print("{stuff} was pulled out".format(stuff = item))
        print("")
        new_house.append(item)
        currently_in_new_house()
